...
{{- /*
  In-cluster Keycloak HTTP base URL used by Lakekeeper for OIDC discovery and
  by lakekeeper-bootstrap to obtain tokens (KEYCLOAK_INTERNAL_ISSUER).

  Uses the default HTTP port (80, implicit) — NOT 8080 — even though Keycloak
  listens on 8080 inside the pod. Why: Keycloak's hostname-v2 provider with
  KC_HOSTNAME_BACKCHANNEL_DYNAMIC=true derives backchannel URLs (jwks_uri,
  token_endpoint in the discovery doc) from the request's Host header but
  STRIPS non-default ports. A client doing discovery via `:8080` gets back
  URLs without any port and then can't reach JWKS. By using the default-port
  URL here, the Host header carries no explicit port, Keycloak emits URLs
  without port, and the JWKS fetch hits port 80 → routes to pod 8080 via the
  auxiliary `http` port on the keycloak Service (see identity service.yaml).
*/}}
...
{{- end }}
{{- printf "http://keycloak.%s.svc.cluster.local" $keycloakNs }}
{{- end }}
