...
def create_client(kc: KeycloakAdmin, client_config: Dict[str, Any]) -> bool:
    """Create or update an OIDC client in the current realm.

    Idempotent + reconciling: if the client already exists we PATCH it with the desired
    config (notably redirectUris / webOrigins) so that re-running the Job after a
    DOMAIN/PROTOCOL change actually fixes stale URIs. Skipping on exist would silently
    leave clients pinned to whatever ENDPOINT was used on the very first deploy.
    """
    client_id = client_config.get('clientId')

    existing_uuid = get_client_uuid(kc, client_id)
    if existing_uuid:
        print(f"Updating existing client '{client_id}' (uuid={existing_uuid}) with current redirectUris/webOrigins...")
        try:
            kc.update_client(existing_uuid, client_config)
            print(f"✅ Client '{client_id}' updated successfully")
            return True
        except KeycloakError as e:
            print(f"Error: Failed to update client '{client_id}': {e}")
            if hasattr(e, 'response_code'):
                print(f"HTTP Code: {e.response_code}")
            if hasattr(e, 'response_body'):
                print(f"Response: {e.response_body}")
            ...
            print(f"Error: Unexpected error updating client: {e}")
            return False
...
def main():
    ...
    # Per-app public hostnames. Each app lives on its own single-label subdomain
    # so the cluster's wildcard DNS (`*.{endpoint}`) resolves it. The bare apex
    # `{domain}` is intentionally avoided in OIDC URIs because it does not resolve
    # on clusters with single-label-only wildcard DNS.
    #   CONSOLE_HOST  -> e.g. app.agentstudio.local      (gui, gateway, swagger)
    #   CATALOG_HOST  -> e.g. catalog.agentstudio.local  (lakekeeper-ui)
    # If unset, fall back to f"app.{domain}" / f"catalog.{domain}" for safe defaults.
    console_host = get_env_or_default("CONSOLE_HOST", f"app.{domain}")
    catalog_host = get_env_or_default("CATALOG_HOST", f"catalog.{domain}")

    print(f"Keycloak URL: {keycloak_url}")
    print(f"Realm: {realm_name}")
    print(f"Domain (base endpoint): {domain}")
    print(f"Console host: {console_host}")
    print(f"Catalog host: {catalog_host}")
    ...
    # Console is served on console_host (e.g. app.agentstudio.local), NOT the apex.
    gui_client_config = {
        ...
        "redirectUris": [
            f"{protocol}://{console_host}/console/*",
            f"{protocol}://{console_host}:8443/console/*",
            f"{protocol}://{catalog_host}/*",
            f"{protocol}://{catalog_host}:8443/*",
            "http://localhost:3000/*"
        ],
        "webOrigins": [
            f"{protocol}://{console_host}",
            f"{protocol}://{console_host}:8443",
            f"{protocol}://{catalog_host}",
            f"{protocol}://{catalog_host}:8443",
            "http://localhost:3000"
        ],
        "attributes": {
            # Keep logout allowlist in lockstep with redirectUris to avoid
            # Invalid redirect uri when port-explicit hosts (e.g. :8443) are used.
    ...
    # Gateway redirects land on console_host (where the API gateway is published).
    ...
    # Lakekeeper UI is served on catalog_host (e.g. catalog.agentstudio.local).
    ...
    # Swagger is mounted under /api-docs on the API gateway (console_host).
    swagger_client_config = {
        ...
        "serviceAccountsEnabled": False,
        "redirectUris": [
            f"{protocol}://{console_host}/api-docs/*",
            f"{protocol}://{console_host}:8443/api-docs/*",
            f"https://{console_host}/api-docs/*",
            f"https://{console_host}:8443/api-docs/*",
    ...
    # Each entry yields one confidential service-account client
    # `agentstudio-<service>` whose secret is written into the
    # `keycloak-oidc-secrets` K8s Secret under `<service>-client-secret`.
    # The corresponding service Deployment's init container
    # (`wait-for-keycloak-setup`) blocks until that key appears, so
    # ANY new service that authenticates to Keycloak MUST be added
    # here or its pod will hang forever waiting on the secret.
    services = [
        "workflow-engine",
        "analytics-engine",
        "config-service",
    ...
    # Look up agent-studio-api resource server UUID for per-project authz.
    # The workflow-engine needs this to call the Authorization Services API.
    api_uuid = get_client_uuid(kc, "agent-studio-api")
    if api_uuid:
        k8s_secrets["resource-server-uuid"] = api_uuid
        print(f"agent-studio-api resource server UUID: {api_uuid}")
    else:
        print("⚠️  Warning: agent-studio-api client not found; per-project authz will not work")

    # Look up agent-studio-svc-config credentials for the workflow-engine's
    # Keycloak Authorization Services client. The realm bootstrap job grants
    # this service account manage-authorization + uma_protection roles, so the
    # workflow-engine reuses it for Admin Authz API calls instead of needing
    # its own role grants.
    svc_config_uuid = get_client_uuid(kc, "agent-studio-svc-config")
    if svc_config_uuid:
        svc_config_secret = get_client_secret(kc, svc_config_uuid)
        if svc_config_secret:
            k8s_secrets["authz-client-id"] = "agent-studio-svc-config"
            k8s_secrets["authz-client-secret"] = svc_config_secret
            print(f"agent-studio-svc-config credentials stored for authz client")
        else:
            print("⚠️  Warning: agent-studio-svc-config secret not retrievable")
    else:
        print("ℹ️  agent-studio-svc-config not found (svcClients may be disabled); "
              "workflow-engine will use its own credentials for authz")
    ...
    print("  - agentstudio-artifact-service (Service Account)")
    print("  - agentstudio-storage-manager (Service Account)")
    print("  - agentstudio-connector-worker (Service Account)")
    print("  - agentstudio-eval-worker (Service Account)")
    print()
    ...
    if k8s_secrets:
        ...
        print("  kubectl rollout restart deployment/artifact-service -n agentstudio")
        print("  kubectl rollout restart deployment/storage-manager -n agentstudio")
        print("  kubectl rollout restart deployment/connector-worker -n agentstudio")
        print("  kubectl rollout restart deployment/workers-eval -n agentstudio")
