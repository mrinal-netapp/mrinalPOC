{{/*
Tier: services (agentstudio-services namespace)
Shared helpers used by subcharts. The names stay in the "nemo.*" namespace
so existing subchart templates that call include "nemo.*" . continue to work.
*/}}
...
{{/*
Namespace for resources in this tier (uses Helm --namespace; kept for compatibility
with subchart templates that call include "nemo.namespace" .).
*/}}
...
{{/*
Gateway-related helpers (gatewayName / gatewayClassName / httprouteName /
effectiveGatewayHostnames) moved to the new edge chart in
deployments/helm/edge/templates/_helpers.tpl as part of the Istio
ingress migration. The services chart no longer ships any
...
{{/* HTTP port (default 80; local overrides to 8080 via global.gatewayHttpPort). */}}
{{- define "nemo.gatewayHttpPort" -}}
{{- $port := 80 -}}
{{- if and .Values.global (index .Values.global "gatewayHttpPort") -}}
  {{- $port = int (index .Values.global "gatewayHttpPort") -}}
{{- end -}}
{{- $port -}}
{{- end }}

{{/* HTTP port suffix: ":N" when port != 80, empty otherwise. */}}
{{- define "nemo.gatewayHttpPortSuffix" -}}
{{- $port := int (include "nemo.gatewayHttpPort" .) -}}
{{- if ne $port 80 -}}{{- printf ":%d" $port -}}{{- end -}}
{{- end }}

{{- define "nemo.endpointUrl" -}}
{{- printf "https://%s%s" (include "nemo.consoleHost" .) (include "nemo.gatewayPortSuffix" .) }}
{{- end }}

{{- define "nemo.endpointUrlHttp" -}}
{{- printf "http://%s%s" (include "nemo.consoleHost" .) (include "nemo.gatewayHttpPortSuffix" .) }}
{{- end }}

{{/*
Keycloak URL helpers — identity tier runs in agentstudio-identity namespace.
Override via keycloak.setup.keycloakUrl in values.yaml.
*/}}
...
{{/*
S3 gateway credentials secret name — created by s3gateway chart in platform namespace.
Synced into services namespace by CI so workers can read it.
*/}}
...
{{- printf "http://config-service.%s.svc.cluster.local:3000" .Release.Namespace }}
{{- end }}
