"""PostgreSQL connector datasource lifecycle."""
...
pytestmark = [
    ...
    pytest.mark.connector_postgresql,
    pytest.mark.postgres,
    allure.feature("PostgreSQL datasource lifecycle"),
]
...
def postgres_settings(integration_settings: IntegrationSettings) -> IntegrationSettings:
    if not integration_settings.has_postgres():
        pytest.skip("Set POSTGRES_* in .env.local")
    return integration_settings


@pytest.mark.parametrize("dm_credential_prereq", ["postgresql"], indirect=True)
@allure.title("PostgreSQL: datasource CRUD + connection test (credential prerequisite)")
def test_postgresql_datasource_lifecycle(
