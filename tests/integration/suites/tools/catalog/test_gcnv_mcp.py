"""Catalog (managed) MCP tools suite — GCNV control-plane (`gcnv_mcp`).

Self-provisioning, GCP-env-gated flow against a live config-service:

    project -> gcp credential -> managed MCP server (gcnv_mcp) -> runtime ready

The suite creates its own GCP credential and managed MCP server (it does not
reuse any pre-provisioned MCP id), verifies each step, then polls the managed
runtime until the pod is running and ready. Teardown (gated by
``INTEGRATION_CLEANUP``) deletes the MCP server, credential, and project.
...
class TestGcnvMcp:
    """Ordered self-provisioning catalog-MCP flow for `gcnv_mcp`."""
    ...
    def test_create_project(self, ctx: GcnvMcpContext) -> None:
        """Create a fresh project that anchors the GCNV managed-MCP flow."""
        name = _underscore_name("e2e_tools_gcnv")
        resp = ctx.config_client.create_project(
            name, metadata={"source": "pytest-tools-gcnv"}
        )
    ...
    def test_add_gcnv_cred(self, ctx: GcnvMcpContext) -> None:
        ...
        request = CredentialCreateRequest(
            name=_underscore_name("gcnv_cred"),
            provider="gcp",
    ...
    def test_get_gcnv_cred_details(self, ctx: GcnvMcpContext) -> None:
    ...
    def test_add_gcnv_mcp(self, ctx: GcnvMcpContext) -> None:
        """Create the managed GCNV MCP server (begins async provisioning)."""
        if not ctx.credential_id:
            pytest.skip("no credential — test_add_gcnv_cred did not succeed")
    ...
    def test_get_gcnv_mcp_details(self, ctx: GcnvMcpContext) -> None:
        """Poll runtime-status until the managed pod is running and ready."""
        if not ctx.mcp_id:
            pytest.skip("no mcp server — test_add_gcnv_mcp did not succeed")
        log.info(
