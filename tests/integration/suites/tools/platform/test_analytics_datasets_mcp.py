"""Platform MCP tools suite — analytics datasets (`analytics_datasets_mcp`).
...
``analytics_datasets_mcp`` platform server through the list, and strictly
...
TARGET_NAME = "analytics_datasets_mcp"
TARGET_CATALOG_ID = "analytics_datasets_mcp"
...
class TestAnalyticsDatasetsMcp:
    """Ordered discover-and-validate flow for the platform `analytics_datasets_mcp`."""
    ...
    def test_create_project(self, ctx: PlatformMcpContext) -> None:
        ...
        name = f"e2e-tools-platform-analytics-{int(time.time())}"
