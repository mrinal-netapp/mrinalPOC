"""Remote MCP tools suite — API-key remote wikipedia MCP.

Registers a remote MCP server (``deploymentType: "remote"``, ``authType:
"api_key"``) pointing at an externally supplied wikipedia MCP URL that expects
the API key in an ``x-api-key`` request header. The key is passed via
``staticHeaders`` so config-service forwards it to the MCP on every call.
Config-service live-connects at create time and requires at least one tool, so
the server is returned ``status: "connected"`` / ``syncStatus: "synced"``.

    project -> remote MCP server (wikipedia, x-api-key) -> validate details

Skips entirely unless both ``WIKIPEDIA_MCP_SERVER_URL`` and
``WIKIPEDIA_MCP_API_KEY`` are set. The transport comes from
``WIKIPEDIA_MCP_CONNECTION_TYPE`` (defaults to ``streamable-http``). Teardown
...
the remote server is unreachable, the API key is rejected, or config-service
cannot register it.
"""
...
_MCP_NAME_PREFIX = "wikipedia_mcp"
...
class TestRemoteMcpWithAuth:
    """Ordered register-and-validate flow for an API-key remote wikipedia MCP."""
    ...
    ) -> RemoteAuthMcpContext:
        """Before/after class: env gate, build client + shared context."""
        if not agent_service_config.wikipedia_mcp_server_url:
            pytest.skip(
                "WIKIPEDIA_MCP_SERVER_URL not set — set it in "
                "tests/integration/.env.local"
            )
        if not agent_service_config.wikipedia_mcp_api_key:
            pytest.skip(
                "WIKIPEDIA_MCP_API_KEY not set — set it in "
                "tests/integration/.env.local"
            )
        config_client = ConfigServiceClient(agent_service_config, integration_settings)
        state = RemoteAuthMcpContext(
    ...
    def test_create_project(self, ctx: RemoteAuthMcpContext) -> None:
        """Create a fresh project that anchors the remote-MCP flow."""
        name = _underscore_name("e2e_tools_remote_wikipedia")
    ...
    def test_add_wikipedia_mcp(self, ctx: RemoteAuthMcpContext) -> None:
        """Register the API-key remote wikipedia MCP server (x-api-key header)."""
        ...
        request = RemoteMcpServerRequest(
            ...
            url=ctx.config.wikipedia_mcp_server_url,
            transport=ctx.config.wikipedia_mcp_connection_type,
            auth_type="api_key",
            static_headers={_API_KEY_HEADER: ctx.config.wikipedia_mcp_api_key},
        )
        url = ctx.config_client.mcp_servers_url(ctx.project_id)
        # Redact the api key from the logged request body.
        logged_body = {
            **request.to_body(),
            "staticHeaders": {_API_KEY_HEADER: "***"},
        }
        resp = ctx.config_client.add_custom_mcp(ctx.project_id, request)
        log_exchange("POST", url, logged_body, resp)
        assert resp.status_code == 201, resp.text
        data = resp.json()
        ctx.mcp_id = data["id"]
        assert ctx.mcp_id, "mcp id missing in response"
        assert data.get("deploymentType") == "remote"
        assert data.get("authType") == "api_key"
        ctx.resources.add_mcp_server(ctx.mcp_id, ctx.project_id)

    def test_validate_wikipedia_mcp_details(self, ctx: RemoteAuthMcpContext) -> None:
        """Fetch the remote server by id and validate its fields + health."""
        if not ctx.mcp_id:
            pytest.skip("no mcp server — test_add_wikipedia_mcp did not succeed")
        ...
        assert data.get("deploymentType") == "remote"
        assert data.get("transport") == ctx.config.wikipedia_mcp_connection_type
        assert data.get("url") == ctx.config.wikipedia_mcp_server_url
        assert data.get("authType") == "api_key"
        static_headers = data.get("staticHeaders") or {}
        assert _API_KEY_HEADER in static_headers, (
            f"expected {_API_KEY_HEADER!r} in staticHeaders, got "
            f"{sorted(static_headers)!r}"
        )
        assert data.get("status") == "connected", (
            f"remote wikipedia MCP not connected: status={data.get('status')!r}, "
            f"syncStatus={data.get('syncStatus')!r}"
        )
        assert data.get("syncStatus") == "synced", (
            f"remote wikipedia MCP not synced: status={data.get('status')!r}, "
            f"syncStatus={data.get('syncStatus')!r}"
        )
