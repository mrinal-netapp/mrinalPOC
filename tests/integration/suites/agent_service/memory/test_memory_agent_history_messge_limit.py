"""Agent-service memory suite — sliding-window message-history limit.

Chained end-to-end flow against a live deployment:

    project -> model -> agent (sliding_window, windowSize=2)
...
agent-service (`AgentServiceClient`). A single session is seeded with three
friends in order on the same agent, accumulating six stored messages (three
user/assistant exchanges). With ``memoryConfig.windowSize`` (and
``memoryContext.message_history_limit``) set to ``_MESSAGE_HISTORY_LIMIT`` (2),
the contract is that ``citations.contextUsed.historyMessagesCount`` caps at the
window once the session holds more messages than it — the oldest exchanges slide
out. The final recall asserts that server-reported integer equals the window.

We assert on ``historyMessagesCount`` rather than the model's prose or
``historyPreview`` because the assistant echoes prior messages cumulatively,
which would defeat any content-based eviction check. State flows between the
ordered methods through a single class-scoped `AgentHistoryLimitContext`.
"""
...
PROJECT_NAME_PREFIX = "e2e-memory-msglimit"
PROJECT_SOURCE = "pytest-agent-memory-msglimit"
...
# messages, comfortably above the two-message window so eviction must occur.
_FRIEND_1 = "Rahul"
_FRIEND_2 = "Aniket"
_FRIEND_3 = "Ajay"

# Window size (in messages) configured via BOTH memory_config.windowSize and
# memoryContext.message_history_limit. The contract is that
# citations.contextUsed.historyMessagesCount equals this value once the session
# holds more messages than the window.
_MESSAGE_HISTORY_LIMIT = 2


@dataclass
class AgentHistoryLimitContext:
...
class TestMemoryAgentHistoryMessageLimit:
    """Ordered sliding-window message-cap flow (methods run in order)."""
    ...
    def ctx(
        ...
        state = AgentHistoryLimitContext(
    ...
    def test_create_and_validate_project(self, ctx: AgentHistoryLimitContext) -> None:
    ...
    def test_add_and_validate_azure_openai_cred(
        self, ctx: AgentHistoryLimitContext
    ) -> None:
    ...
    def test_add_and_validate_model(self, ctx: AgentHistoryLimitContext) -> None:
    ...
    def test_create_agent_with_memory(self, ctx: AgentHistoryLimitContext) -> None:
        """Create an agent with a small sliding-window memory.

        Test scenario:
            Skips if no model exists. POSTs an ``AgentCreationRequest`` using
            ``sliding_window`` memory with ``windowSize`` /
            ``message_history_limit`` set to ``_MESSAGE_HISTORY_LIMIT`` (2), then
            stores the returned agent id on the context.
        ...
        request = AgentCreationRequest(
            name=unique_name("memory_msglimit_agent"),
            role="you are an helpful memory agent",
            system_prompt="Give very short answers",
            model_id=ctx.model_id,
            memory_type="sliding_window",
            # windowSize is the real sliding-window cap that trims loaded
            # history; the eviction recall relies on it (not
            # message_history_limit) to bound historyMessagesCount.
            memory_config={"windowSize": _MESSAGE_HISTORY_LIMIT},
            memory_context={
                "enabled": True,
                "message_retention_policy": "sliding_window",
                "message_history_limit": _MESSAGE_HISTORY_LIMIT,
            },
        )
    ...
    def test_get_agent_memory_configuration(
        ...
        Test scenario:
            ...
            config-service and validates memory persistence.

        Validation we are covering:
            Asserts HTTP 200 and that memory fields match the creation request:
            ``memoryType=sliding_window``, ``memoryConfig.windowSize`` and
            ``memoryContext.message_history_limit`` both equal
            ``_MESSAGE_HISTORY_LIMIT``.
        """
        if not ctx.agent_id:
        ...
        assert data.get("memoryType") == "sliding_window"
        assert data.get("memoryConfig", {}).get("windowSize") == _MESSAGE_HISTORY_LIMIT
        memory_context = data.get("memoryContext") or {}
        assert memory_context.get("enabled") is True
        assert memory_context.get("type") == "window"
        assert (
            memory_context.get("message_window_limit") == _MESSAGE_HISTORY_LIMIT
        )

    def test_invoke_agent_seed_friend_1(self, ctx: AgentHistoryLimitContext) -> None:
        ...
        if "memoryDegraded" in data:
            assert data["memoryDegraded"] is False, (
                "session memory degraded on the first seed turn"
            )

    def test_invoke_agent_seed_friend_2(self, ctx: AgentHistoryLimitContext) -> None:
        ...
        Test scenario:
            ...
            states that ``Aniket`` is a friend. This is the first of two newer
            facts that push ``Rahul`` out of the sliding window.

        Validation we are covering:
            Asserts HTTP 200, non-empty output, that the turn ran on the seeded
            session (scoped ``contextUsed.sessionId`` ends with it), and that
            ``memoryDegraded`` (if present) is ``False``.
        """
        if not ctx.agent_id or not ctx.session_id:
            pytest.skip("no session — test_invoke_agent_seed_friend_1 did not succeed")
        request = AgentInvocationRequest(
        ...
        assert scoped_session_id.endswith(ctx.session_id), (
            f"Aniket seed ran on a different session: {scoped_session_id!r} "
            f"does not match {ctx.session_id!r}"
        )
        if "memoryDegraded" in data:
            assert data["memoryDegraded"] is False, (
                "session memory degraded on the Aniket seed turn"
            )

    def test_invoke_agent_seed_friend_3(self, ctx: AgentHistoryLimitContext) -> None:
        """Seed a third friend (Ajay) into the same session.

        Test scenario:
            Skips if the session is missing. Replays the session id and states
            that ``Ajay`` is a friend. With the two-message window the session
            now retains only the most recent exchanges; the original ``Rahul``
            exchange is expected to have slid out.

        Validation we are covering:
            Asserts HTTP 200, non-empty output, that the turn ran on the seeded
            session, and that ``memoryDegraded`` (if present) is ``False``.
        """
        if not ctx.agent_id or not ctx.session_id:
            pytest.skip("no session — test_invoke_agent_seed_friend_1 did not succeed")
        request = AgentInvocationRequest(
        ...
        scoped_session_id = context_used.get("sessionId") or ""
        assert scoped_session_id.endswith(ctx.session_id), (
            f"Ajay seed ran on a different session: {scoped_session_id!r} "
            f"does not match {ctx.session_id!r}"
        )
        if "memoryDegraded" in data:
            assert data["memoryDegraded"] is False, (
                "session memory degraded on the Ajay seed turn"
            )

    def test_invoke_agent_recall_after_eviction(
        self, ctx: AgentHistoryLimitContext
    ) -> None:
        """Sliding window caps loaded history at the configured limit.
        ...
        Test scenario:
            ...
            asks the agent what messages it has received, then reads the
            server-reported ``citations.contextUsed.historyMessagesCount``.

        Validation we are covering:
            Asserts HTTP 200, non-empty output, that the turn ran on the seeded
            session, and — the sliding-window guarantee — that
            ``historyMessagesCount`` equals ``_MESSAGE_HISTORY_LIMIT`` (the
            configured ``windowSize`` / ``message_history_limit``). We assert on
            this server-reported integer rather than the model's prose or
            ``historyPreview`` because the assistant echoes every prior message
            cumulatively, which would defeat any content-based eviction check.
        """
        if not ctx.agent_id or not ctx.session_id:
            pytest.skip("no session — test_invoke_agent_seed_friend_1 did not succeed")
        request = AgentInvocationRequest(
        ...
        if "memoryDegraded" in data:
            assert data["memoryDegraded"] is False, (
                "session memory degraded on the eviction recall turn"
            )
        context_used = (data.get("citations") or {}).get("contextUsed") or {}
        scoped_session_id = context_used.get("sessionId") or ""
        assert scoped_session_id.endswith(ctx.session_id), (
            f"eviction recall ran on a different session: {scoped_session_id!r} "
            f"does not match {ctx.session_id!r}"
        )
        # Deterministic eviction proof: by this turn the session holds six stored
        # messages, but with windowSize=2 / message_history_limit=2 the sliding
        # window caps the loaded history at _MESSAGE_HISTORY_LIMIT (the oldest
        # exchanges slid out).
        ...
        assert history_count == _MESSAGE_HISTORY_LIMIT, (
            f"sliding window not enforced: historyMessagesCount={history_count} "
            f"does not equal the configured window of {_MESSAGE_HISTORY_LIMIT}; "
