"""Agent-service coordinate (magentic-backed) orchestration policy suite.

Chained end-to-end flow against a live deployment:

    project -> model -> three agents -> coordinate team (manager) -> team invoke
...
into a team with the ``coordinate`` orchestration policy plus an inline manager
that decomposes the task, delegates to the experts, and synthesizes a final
answer.

The team is first validated on creation (config-service echoes
``orchestrationPolicy == "coordinate"`` and the inline manager). It is then
invoked: because agent-service aliases ``coordinate`` to the ``magentic``
runtime orchestrator, the invoke response `metadata.orchestration_type` echoes
``"magentic"`` (not ``"coordinate"``), and `metadata.agent_names` includes all three
members. State flows between the
ordered methods through a single class-scoped `CoordinateContext`.

See ``test_orchestration_concurrent.py`` and ``test_orchestration_sequential.py``
for the other policy counterparts.
"""
...
PROJECT_NAME_PREFIX = "e2e-orch-coordinate"
PROJECT_SOURCE = "pytest-agent-orchestration-coordinate"
...
_MANAGER_NAME = "science-orchestrator"
...
class TestOrchestrationCoordinate:
    """Ordered coordinate-team orchestration flow (methods run in definition order)."""
    ...
    def test_create_and_validate_project(self, ctx: CoordinateContext) -> None:
    ...
    def test_add_and_validate_model(self, ctx: CoordinateContext) -> None:
    ...
    def test_create_agents(self, ctx: CoordinateContext) -> None:
    ...
    def test_create_team_coordinate(self, ctx: CoordinateContext) -> None:
        """Create a coordinate team with an inline manager over three members.

        Test scenario:
            Skips when any of the three agents is missing. POSTs a team to
            config-service with ``orchestration_policy="coordinate"``, the three
            member agent ids, and an inline manager that decomposes, delegates,
            and synthesizes; stores the returned team id on the context.

        Validation we are covering:
            The create returns HTTP 201, a non-empty team id is present, the
            echoed ``orchestrationPolicy`` equals ``"coordinate"``, exactly three
            members are returned, and the inline manager is echoed back by name.
        """
        if not (
        ...
        team_request = TeamCreationRequest(
            name=unique_name("e2e-orch-coordinate-team"),
            ...
            manager=TeamManagerConfig(
                name=_MANAGER_NAME,
                system_prompt=(
                    "You are the manager. Decompose the question, delegate to the physics, "
                    "chemistry, and biology experts, and synthesize a final answer."
                ),
                model_id=ctx.model_ids[0],
            ),
        )
        ...
        assert manager.get("name") == _MANAGER_NAME, (
            f"expected inline manager echoed back; got {manager!r}"
        )
        ctx.resources.add_team(ctx.team_id, ctx.project_id)

    def test_invoke_team_coordinate(self, ctx: CoordinateContext) -> None:
        """Invoke the coordinate team and confirm the magentic-backed run.
        ...
        Validation we are covering:
            ...
            ``metadata.orchestration_type`` echoes ``"magentic"`` (coordinate
            aliases to magentic), and ``metadata.agent_names`` includes all three
            member names.
        """
        if not ctx.team_id:
            pytest.skip("no team — test_create_team_coordinate did not succeed")
