"""Agent-service sequential orchestration policy suite.

Chained end-to-end flow against a live deployment:

    project -> model -> two agents -> sequential team -> team invoke

Provisioning and team creation use config-service (`ConfigServiceClient`); the
team invoke uses agent-service (`AgentServiceClient`). A two-member team is
created with the ``sequential`` orchestration policy (no manager), then invoked;
the invoke response `metadata.orchestration_type` must echo ``"sequential"``,
proving the orchestration policy was applied. State flows between the ordered
methods through a single class-scoped `OrchestrationContext`.

See ``test_orchestration_concurrent.py`` for the concurrent-policy counterpart.
"""
...
PROJECT_NAME_PREFIX = "e2e-orchestration"
PROJECT_SOURCE = "pytest-agent-orchestration"
...
_ORCHESTRATION_POLICY = "sequential"
...
class TestOrchestrationSequential:
    """Ordered sequential-team orchestration flow (methods run in definition order)."""
    ...
    def test_create_and_validate_project(self, ctx: OrchestrationContext) -> None:
    ...
    def test_add_and_validate_model(self, ctx: OrchestrationContext) -> None:
    ...
    def test_create_agents(self, ctx: OrchestrationContext) -> None:
        """Create the two member agents for the sequential team.

        Test scenario:
            Skips when no model exists. POSTs two agents to config-service (an
            ``assistant`` and a ``researcher``) bound to the provisioned model,
            storing both agent ids on the shared context.

        Validation we are covering:
            Each create returns HTTP 201 and the returned agent id matches the
            ``ag-xxxxxxxx`` id pattern.
        """
        if not ctx.model_ids:
            pytest.skip("no model — test_add_and_validate_model did not succeed")
        agent1_request = AgentCreationRequest(
            name=unique_name("e2e-orch-agent1"),
            role="assistant",
            system_prompt="You are a helpful assistant.",
            model_id=ctx.model_ids[0],
        )
        agent_url = ctx.config_client.agents_url(ctx.project_id)
        resp1 = ctx.config_client.create_agent(ctx.project_id, agent1_request)
        log_exchange("POST", agent_url, agent1_request.to_body(), resp1)
        assert resp1.status_code == 201, resp1.text
        ctx.agent_id = resp1.json()["id"]
        ...
        agent2_request = AgentCreationRequest(
            name=unique_name("e2e-orch-agent2"),
            role="researcher",
        ...
        assert AGENT_ID_RE.match(ctx.agent_id_2), f"unexpected agent id: {ctx.agent_id_2!r}"
        ctx.resources.add_agent(ctx.agent_id_2, ctx.project_id)

    def test_create_team_sequential(self, ctx: OrchestrationContext) -> None:
        """Create a two-member team using the ``sequential`` orchestration policy.

        Test scenario:
            Skips when either agent is missing. POSTs a team to config-service
            with ``orchestration_policy="sequential"`` and the two member agent
            ids, then stores the returned team id on the shared context.

        Validation we are covering:
            The create returns HTTP 201, a non-empty team id is present, the
            echoed ``orchestrationPolicy`` equals ``"sequential"``, and exactly
            two members are returned.
        """
        if not ctx.agent_id or not ctx.agent_id_2:
            pytest.skip("no agents — test_create_agents did not succeed")
        team_request = TeamCreationRequest(
            name=unique_name("e2e-orch-team"),
    ...
    def test_invoke_team_orchestration_sequential(self, ctx: OrchestrationContext) -> None:
        """Invoke the sequential team and confirm the policy was applied.

        Test scenario:
            Skips when no team exists. POSTs an invoke request to agent-service
            with a single-sentence summarization prompt and reads the response
            body and metadata.

        Validation we are covering:
            The invoke returns HTTP 200, ``output`` is non-empty, and
            ``metadata.orchestration_type`` echoes ``"sequential"``, proving the
            sequential policy drove the run.
        """
        if not ctx.team_id:
