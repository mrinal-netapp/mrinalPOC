#!/bin/bash
# Script to retrieve and update the workflow-engine Keycloak client secret in Kubernetes
...
CLIENT_ID="${CLIENT_ID:-agentstudio-workflow-engine}"
...
kubectl create secret generic "$SECRET_NAME" \
  --from-literal=workflow-engine-client-id="$CLIENT_ID" \
  --from-literal=workflow-engine-client-secret="$CLIENT_SECRET" \
  --namespace="$NAMESPACE" \
...
echo ""
echo "The workflow-engine should now be able to authenticate with Keycloak."
echo "You may need to restart the workflow-engine pod for the changes to take effect:"
echo "  kubectl rollout restart deployment/workflow-engine -n $NAMESPACE"
