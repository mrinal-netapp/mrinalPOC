...
export function classifyDuckDBType(dbType: string): ColumnTypeCategory {
  ...
  if (['DOUBLE', 'FLOAT', 'REAL', 'DECIMAL', 'NUMERIC'].some((n) => t.includes(n))) return 'float';
  if (['VARCHAR', 'TEXT', 'CHAR', 'BLOB', 'UUID', 'ENUM'].some((n) => t.includes(n))) return 'string';
  if (['TIMESTAMP', 'DATE', 'TIME', 'INTERVAL'].some((n) => t.includes(n))) return 'temporal';
