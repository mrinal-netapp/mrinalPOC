...
export const PROVIDER_PRESETS: Record<string, ProviderPreset> = {
  ...
  aws: {
    ...
    secretFields: [
      { key: 'access_key_id', label: 'Access Key ID', type: 'text', required: true, placeholder: 'AKIA...' },
      { key: 'secret_access_key', label: 'Secret Access Key', type: 'password', required: true, placeholder: 'Secret access key' },
      { key: 'session_token', label: 'Session Token', type: 'password', required: false, placeholder: 'Temporary STS session token (optional)' },
    ],
    metadataFields: [
  ...
  google: {
    ...
    secretFields: [
      {
        ...
        type: 'password',
        required: true,
        placeholder: 'Paste service account key JSON or use Upload JSON file',
        multiline: true,
        jsonFileUpload: true,
      },
    ],
    metadataFields: [
      ...
      { key: 'region', label: 'Region', placeholder: 'us-central1', required: true },
      { key: 'project_number', label: 'Project Number (fine-tuned models only)', placeholder: '123456789012', required: false },
    ],
  },
  gemini: {
    label: 'Google Gemini',
    category: 'AI / LLM',
    secretFields: [
      { key: 'api_key', label: 'API Key', type: 'password', required: true, placeholder: 'Google AI Studio API key' },
    ],
  },
  cohere: {
    label: 'Cohere',
    category: 'AI / LLM',
    secretFields: [
      { key: 'api_key', label: 'API Key', type: 'password', required: true, placeholder: 'Cohere API key' },
    ],
  },
  perplexity: {
    label: 'Perplexity',
  ...
  huggingface: {
    ...
    category: 'AI / LLM',
    secretFields: [
      { key: 'api_key', label: 'API Key', type: 'password', required: true, placeholder: 'hf_...' },
    ],
  },
  fireworks: {
    label: 'Fireworks AI',
    category: 'AI / LLM',
    secretFields: [
      { key: 'api_key', label: 'API Key', type: 'password', required: true, placeholder: 'fw_...' },
    ],
  },
  tavily: {
    label: 'Tavily',
