/**
 * HTTP-level tests for PostgreSQL datasource/dataset validation wiring (no Postgres DB).
 *
 * Mirrors mysql.routes.unit.test.ts: POST/PUT hit the same validator chains as production routes.
 *
 * Note: PUT /datasources does not re-run provider-catalog validation on connector_config
 * (same as S3/MySQL); only shape checks from updateDataSourceValidator apply.
...
import { basePostgresqlConnectorCreate, baseAcquiredPostgresqlDataset } from './helpers/postgresqlFixtures';
...
test('POST /datasources: valid PostgreSQL connector returns 201', async () => {
  const res = await listen(buildDataSourceValidationApp(), 'POST', DS_BASE, basePostgresqlConnectorCreate());
...
test('POST /datasources: missing host returns 400', async () => {
  ...
  const res = await listen(
    ...
    'POST',
    DS_BASE,
    basePostgresqlConnectorCreate({ connector_config: cfg }),
  );
  assert.equal(res.status, 400);
...
test('POST /datasets: valid acquired PostgreSQL dataset returns 201', async () => {
  const res = await listen(buildDataSetValidationApp(), 'POST', SET_BASE, baseAcquiredPostgresqlDataset());
  assert.equal(res.status, 201);
  assert.equal(res.body?.validated, true);
});
...
test('POST /datasets: empty description returns 201', async () => {
  const res = await listen(
    ...
    'POST',
    SET_BASE,
    baseAcquiredPostgresqlDataset({ description: '' }),
  );
  assert.equal(res.status, 201);
});
...
test('POST /datasets: acquired without origin returns 400 errors array', async () => {
  const res = await listen(
    ...
    'POST',
    SET_BASE,
    baseAcquiredPostgresqlDataset({ originConnector: undefined }),
  );
  assert.equal(res.status, 400);
...
test('PUT /datasets/:id: sqlQuery and sourceDatabase patch returns 200', async () => {
  const res = await listen(buildDataSetValidationApp(), 'PUT', `${SET_BASE}/ds-pg00001`, {
