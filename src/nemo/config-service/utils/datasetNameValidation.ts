...
/**
 * Sanitizes a dataset name to make it a valid table identifier
 * @param datasetName - The dataset name to sanitize
 * @returns Sanitized dataset name that is guaranteed to be valid
 */
export function sanitizeDatasetName(datasetName: string): string {
  ...
  let sanitized = datasetName.trim().toLowerCase()
    .replace(/[^a-z0-9_-]/g, '_') // Replace invalid chars with underscore
    .replace(/_+/g, '_') // Collapse multiple underscores
    .replace(/^_+|_+$/g, '') // Remove leading/trailing underscores
