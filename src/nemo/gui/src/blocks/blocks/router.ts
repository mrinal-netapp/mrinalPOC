...
export const RouterBlock: BlockConfig = {
  ...
  `,
  category: 'blocks',
  bgColor: '#A855F7', // Purple-500 - AI/ML theme
  icon: ConnectIcon,
  subBlocks: [
    {
      id: 'prompt',
      ...
      options: () => {
        try {
          ...
          return allModels.map((model) => {
            ...
            const result: { label: string; id: string; icon?: React.ComponentType<{ className?: string }> } = {
              label: model,
              id: model,
            }
            if (icon) {
  ...
  tools: {
    ...
    config: {
      tool: (params: Record<string, any>) => {
        ...
        // Try to get tool from provider mapping
        const tool = getAllModelProviders()[model as ProviderId]
        if (tool) {
          return tool
        }
        
        // Fallback: Infer tool ID from model name if provider mapping is not available
        ...
        if (modelLower.startsWith('grok-') || modelLower.includes('xai')) {
          return 'xai_chat'
        }
        ...
        // Default to openai_chat for unknown models
        return 'openai_chat'
      },
    },
  },
